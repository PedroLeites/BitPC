...
class Pedidos_Model extends Model
{
    ...
    public function get()
    {
        ...
        try {
            ...
            $query = $this->db->connect()->query('SELECT p.id as IDPedidos, nombre, FechaCom FROM pedidos p INNER JOIN personas pe on p.ID=pe.ID');
            while ($row = $query->fetch()) {
                $articulos = new Pedidos();
                $articulos->IDPedidos = $row['IDPedidos'];

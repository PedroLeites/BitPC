...
class Articulos_Model extends Model
{
    ...
    public function verArticulo($id)
    {
        $articulo = null;
        try {
            $query = $this->db->connect()->prepare('SELECT id, nombre, descripcion, precio FROM productos WHERE id=:id');
    ...
    public function actualizar($articulo)
    {
        ...
        try {
            $query = $pdo->prepare('UPDATE productos SET nombre=:nombre, descripcion=:descripcion, precio= :precio, url_foto = :url_foto WHERE id= :id');
